...
class BuddyPress_Invite_Codes {
	...
	public function bp_include() {
		...
		if ( $this->meets_requirements() ) {
    		...
    		require_once( $this->directory_path . 'includes/class-bp-invite-codes-frontend.php' );
	...
	public function enqueue_scripts() {
		...
		if ( isset( $bp->current_component ) && 'groups' == $bp->current_component ) {
			wp_enqueue_script( 'sweetalert2', $this->directory_url . 'assets/js/sweetalert2.min.js', array( 'jquery' ), $this->version );
            wp_enqueue_style( 'sweetalert2', $this->directory_url . 'assets/css/sweetalert2.min.css', array(), $this->version );
            wp_enqueue_script( 'bp-invite-codes', $this->directory_url . 'js/bp-invite-codes.js', array( 'jquery' ), $this->version );
	...
	public function maybe_disable_plugin() {
		...
		if ( !$this->meets_requirements() ) {
			...
			if ( !class_exists( 'BadgeOS' ) || !function_exists( 'badgeos_get_user_earned_achievement_types' ) ) {
				echo '<p>' . sprintf( __( 'BuddyPress Invite Codes requires <a href="%s">BadgeOS</a> to be activated.', 'bp-invite-codes' ), 'https://wordpress.org/plugins/badgeos/' ) . '</p>';
			}
			elseif ( !class_exists( 'BuddyPress' ) ) {
				echo '<p>' . sprintf( __( 'BuddyPress Invite Codes requires <a href="%s">BuddyPress</a> to be activated.', 'bp-invite-codes' ), 'https://wordpress.org/plugins/buddypress/' ) . '</p>';
			}
			elseif ( !class_exists( 'BP_Groups_Group' ) || !bp_is_active( 'groups' ) ) {
				echo sprintf( __( '<p>BuddyPress Invite Codes requires BuddyPress <a href="%s">User Groups Component</a> to be enabled, please enable it and activate this plugin.</p>', 'bp-invite-codes' ), admin_url( 'options-general.php?page=bp-components' ) );
			}
