...
function bp_invite_codes_bp_after_signup_profile_fields()
{
    ...
    $require_code = get_option('bp_invite_codes_require_code', 'No');

    if (strtolower($require_code) == 'yes') {
        $entered_code = '';
        ...
        <div class="register-section">
            <div class="editfield">
                ...
                <input type="text" name="bp_invite_code" id="bp_invite_code"
                       value="<?php echo esc_attr($entered_code); ?>"/>
            </div>
        </div>
        <?php
...
function bp_invite_codes_bp_core_signup_user( $user_id, $user_login, $user_password, $user_email, $usermeta ) {
	...
	if ( isset( $_POST[ 'bp_invite_code' ] ) && !empty( $_POST[ 'bp_invite_code' ] ) ) {
		...
		// Multisite activation
			...
			delete_option( 'bp_invite_code_' . md5( $user_email ) );
			add_option( 'bp_invite_code_' . md5( $user_email ), $_POST[ 'bp_invite_code' ], '', 'no' );
		}
	}
...
function bp_invite_codes_bp_core_activate_user( $user_id ) {
	...
	$invite_code = get_option( 'bp_invite_code_' . md5( $user->user_email ) );

	if ( !empty( $invite_code ) ) {
		$return = bp_invites_code_join( $invite_code, 0, $user_id );

		if ( 'join' != $return ) {
			bp_invites_code_join( null, 0, $user_id );
		}

		delete_option( 'bp_invite_code_' . md5( $user->user_email ) );
	}
	else {
		bp_invites_code_join( null, 0, $user_id );
	}
...
function bp_invite_codes_bp_core_activate_blog( $blog_id, $user_id ) {
	...
	bp_invite_codes_bp_core_activate_user( $user_id );
...
function bp_invite_codes_group_admin_action( $group_id ) {
    ...
    $_raw_invite_code = $_POST[ 'bp_invite_codes_code' ];

    if( !empty($_raw_invite_code) ){

	    $pattern = "/^[a-zA-Z0-9_]+$/";
        if(!preg_match($pattern, $_raw_invite_code))   {
            $message = sprintf(
                __( 'Please use alphanumeric characters and underscore for invite code', 'bp-invite-codes' )
            );
 ...
 * @since  1.1.0
 */
function bp_invites_code_join( $code = null, $group_id = 0, $user_id = 0 ) {

	$group_ids = array();

	if ( null === $code && $user_id && !$group_id ) {
		$return = 'join';
		...
		$group_ids = array_map( 'absint', $bp_invite_codes_default_bp_groups );
...
function bp_invite_codes_get_code( $group_id = null, $entered_code = null, $return = null ) {
	...
	// check if entered code can be used
        ...
        $invite_code_expiry=0;
		$invite_limit = (int) get_post_meta( $code_post_id, '_bp_invite_codes_limit', true );
        $invite_code_expiry_date = get_post_meta( $code_post_id, '_bp_invite_codes_expiration', true );
        if (!empty($invite_code_expiry_date)) {
            $invite_code_expiry = strtotime($invite_code_expiry_date . '+1 day'); //Plus one day so code can be expired next day 12:00 midnight
        }
		$invites_used = (int) get_post_meta( $code_post_id, '_bp_invite_codes_used', true );
		...
		elseif ( $invite_code_expiry > 0 && time() > $invite_code_expiry ) {
...
function bp_invite_codes_bp_get_group_join_button( $button ) {
	...
	if ( 'join_group' == $button[ 'id' ] || 'request_membership' == $button[ 'id' ] ) {
		...
		if ( bp_invite_codes_get_code( $group_id, null, 'code' ) ) {
			// echo $code; // Debug
			$button[ 'link_class' ] = $button[ 'link_class' ] . ' class_bp_invite_codes ';
