...
function bp_invite_codes_meta_boxes() {
	...
	add_meta_box( 'bp_invite_codes_groups_meta_box', __( 'Apply invite code on', 'bp-invite-codes' ), 'bp_invite_codes_groups_checkboxes', 'bp-invite-codes', 'side', 'low' );
...
function bp_invite_codes_groups_checkboxes( $post ) {
    ...
    ?>
    <p class="description"><?php _e('Select which BuddyPress group(s) will require this invite code.', 'bp-invite-codes'); ?></p>
    ...
    <p class="description"><?php _e('Note: You can only apply one invite code on a group.', 'bp-invite-codes'); ?></p>
<?php
	wp_reset_postdata();
...
function bp_invite_codes_groups_update( $post_id, $post ) {
    ...
    if (array_key_exists('bp_invite_codes_groups', $_POST)) {
        update_post_meta(
            $post_id,
            '_bp_invite_codes_groups',
            $_POST['bp_invite_codes_groups']
        );
    }

    foreach($_POST['bp_invite_codes_groups'] as $selected_group_id) {
        $group_invite_code_id = groups_get_groupmeta( $selected_group_id, '_bp_invite_codes_post_id' );

        if(!empty($group_invite_code_id) && $group_invite_code_id != $post_id ) {

            $invite_code_selected_gids = get_post_meta( $group_invite_code_id, '_bp_invite_codes_groups', true );

            if( !empty($invite_code_selected_gids) && in_array($selected_group_id, $invite_code_selected_gids) ) {
                if (($key = array_search($selected_group_id, $invite_code_selected_gids)) !== false) {
                    unset($invite_code_selected_gids[$key]);
                }

                update_post_meta( $group_invite_code_id, '_bp_invite_codes_groups', $invite_code_selected_gids );
            }
        }

        groups_update_groupmeta( $selected_group_id, '_bp_invite_codes_post_id', $post_id );
    }
...
function bp_invite_codes_meta_box(  ) {
	...
	$cmb_obj = new_cmb2_box( array(
		...
		'title'         => esc_html__( 'Invite Code Data', 'bp-invite-codes' ),
	...
	$cmb_obj->add_field(array(
		'name' => __( 'Usage Limit', 'bp-invite-codes' ),
		'desc' => __( 'Max number of times this code can be used, leave blank for no limit.', 'bp-invite-codes' ),
		'id' => $prefix . 'limit',
		'type' => 'text_small',
	));
	
	$cmb_obj->add_field(array(
		'name' => __( 'Current Usage', 'bp-invite-codes' ),
	...
	$cmb_obj->add_field(array(
		'name' => __( 'Expiry Date', 'bp-invite-codes' ),
		'desc' => __( 'Date on which code expires, leave blank for no expiry.', 'bp-invite-codes' ),
